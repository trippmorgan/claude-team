...
class ClaudeTeamHub {
  ...
  private async handleIncomingQuery(msg: TeamMessage) {
    ...
    const autoRespond = vscode.workspace.getConfiguration('claudeTeam').get<boolean>('autoRespond', true);
    ...
    const action = await vscode.window.showInformationMessage(
      'Query from ' + msg.fromWindow + ': ' + msg.content.substring(0, 50) + '...',
      'Auto-respond', 'View', 'Ignore'
    );
