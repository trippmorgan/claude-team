...
export enum QueryIntent {
  STATUS_CHECK = 'status_check',       // "What are you working on?"
  CONTEXT_REQUEST = 'context_request', // "Share your context"
  CODE_REQUEST = 'code_request',       // "Show me the User class"
  ERROR_HELP = 'error_help',           // "I'm seeing this error..."
  INTERFACE_REQUEST = 'interface_request', // "What's the interface for X?"
  DEPENDENCY_CHECK = 'dependency_check',   // "Are you using library X?"
  REVIEW_REQUEST = 'review_request',   // "Can you review this code?"
  HELP_REQUEST = 'help_request',       // "Can you help with X?"
  TASK_DELEGATION = 'task_delegation', // "Please implement X"
  SYNC_REQUEST = 'sync_request',       // "Give me a status update"
  GENERAL_QUESTION = 'general_question', // Generic questions
  UNKNOWN = 'unknown'
}
...
interface IntentPattern {
  ...
  patterns: RegExp[];
  keywords: string[];
  weight: number;  // Priority weight for ambiguous matches
}
...
export class QueryClassifier {
  ...
  classify(query: string): ClassificationResult {
    ...
    const results: { intent: QueryIntent; score: number; pattern?: string }[] = [];
    ...
    for (const intentDef of this.intentPatterns) {
      ...
      const keywordMatches = intentDef.keywords.filter(kw =>
   ...
   */
  private getStrategyForIntent(intent: QueryIntent): string {
    const strategyMap: Record<QueryIntent, string> = {
   ...
   */
  requiresApproval(intent: QueryIntent): boolean {
   ...
   */
  isInstantResponse(intent: QueryIntent): boolean {
