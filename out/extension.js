...
class ClaudeTeamHub {
    ...
    async handleIncomingQuery(msg) {
        ...
        this.log('└──────────────────────────────────────────────────────────────');
        // Check auto-respond setting (default TRUE for seamless MCP integration)
        const autoRespond = vscode.workspace.getConfiguration('claudeTeam').get('autoRespond', true);
        this.log('[LOCAL] autoRespond setting = ' + autoRespond);
        if (autoRespond) {
            // Auto-respond with context
            this.log('[LOCAL] *** AUTO-RESPONDING ***');
            const response = this.generateContextResponse(msg.content);
            this.log('[LOCAL] Generated response (' + response.length + ' chars)');
            this.sendResponse(msg.id, msg.fromWindow, response);
            vscode.window.showInformationMessage('Auto-responded to query from ' + msg.fromWindow);
            return;
        }
        // Show notification with options
        const action = await vscode.window.showInformationMessage('Query from ' + msg.fromWindow + ': ' + msg.content.substring(0, 50) + '...', 'Auto-respond', 'View', 'Ignore');
        if (action === 'View') {
        ...
        if (action === 'Auto-respond') {
            const response = this.generateContextResponse(msg.content);
            this.sendResponse(msg.id, msg.fromWindow, response);
            vscode.window.showInformationMessage('Response sent!');
        }
    }
