...
exports.ContextStrategy = void 0;
 ...
 */
class ContextStrategy extends responseStrategy_1.BaseStrategy {
    name = 'context';
    description = 'Provides workspace context and status information';
    priority = 100; // High priority - fast responses
    supportedIntents = [
        queryClassifier_1.QueryIntent.STATUS_CHECK,
        queryClassifier_1.QueryIntent.CONTEXT_REQUEST,
        queryClassifier_1.QueryIntent.SYNC_REQUEST,
        queryClassifier_1.QueryIntent.DEPENDENCY_CHECK
    ];
    async generate(query, context) {
        const intent = query.classification.intent;
        switch (intent) {
            case queryClassifier_1.QueryIntent.STATUS_CHECK:
                return this.generateStatusResponse(query, context);
            case queryClassifier_1.QueryIntent.CONTEXT_REQUEST:
                return this.generateContextResponse(query, context);
            case queryClassifier_1.QueryIntent.SYNC_REQUEST:
                return this.generateSyncResponse(query, context);
            case queryClassifier_1.QueryIntent.DEPENDENCY_CHECK:
    ...
    generateStatusResponse(_query, context) {
    ...
    generateContextResponse(_query, context) {
        const editor = vscode.window.activeTextEditor;
        let response = this.formatHeader(context);
    ...
    generateSyncResponse(_query, context) {
        let response = this.formatHeader(context);
    ...
    async generateDependencyResponse(query, context) {
        let response = this.formatHeader(context);
