...
const chromeClients = new Map<string, ChromeClient>();
const pendingRequests = new Map<string, PendingRequest>();
const capturedData = new Map<string, any>();
      ...
      switch (message.type) {
        ...
        case 'tabs_update':
          ...
          if (client && message.tabs) {
            client.tabs.clear();
            message.tabs.forEach((tab: TabInfo) => {
...
async function forwardToHub(eventType: string, data: any) {
  try {
...
function sendToChromeAndWait(clientId: string, command: any, timeoutMs = 30000): Promise<any> {
...
function broadcastToChrome(message: any) {
...
const server = new Server(
  { name: 'browser-bridge', version: '1.0.0' },
