...
export class Orchestrator {
    ...
    // AGENT MANAGEMENT
    // ---------------------------------------------------------------------------
    updateAgentsFromWindowList(windows) {
        for (const w of windows) {
            if (w.id === 'claude-code-mcp' || w.id === 'orchestrator')
                continue;
    ...
    broadcast(message, category = 'update') {
        if (this.hubSocket?.readyState === WebSocket.OPEN) {
            this.hubSocket.send(JSON.stringify({
